...
+++ Custom Post Type: Lösungen
+++ Custom Post Type: Termine
+++ Custom Post Type: Anbieter
- Subscribe & Connect
- Archive Title
...
add_action( 'init', 'ao_create_produkte' );

function ao_create_produkte() {
		...
		'capability_type'    => 'post',
			'has_archive' => false,
			'rewrite' => array('slug' => 'apple-business-produkte'),
			'taxonomies' => array('post_tag'),
			'supports' => array( 'title', 'custom-fields', 'thumbnail', 'excerpt', 'revisions', 'page-attributes', 'comments', 'post-formats', 'trackbacks'  )
			);

register_post_type( 'produkte', $args );
}

			// "Produkte" Custom Taxonomy
		...
		$labels = array(
			...
			'parent_item' => __( '&Uuml;bergeordnete Produkt-Kategorie' ),
			'parent_item_colon' => __( '&Uuml;bergeordnete Produkte:' ),
			'edit_item' => __( 'Produkt-Kategorie bearbeiten' ),
			'update_item' => __( 'Produkt-Kategorie Update' ),
			'add_new_item' => __( 'Neue Produkt-Kategorie' ),
			'new_item_name' => __( 'Produkt-Kategorie neu benennen' ),
			'menu_name' => __( 'Prod.-Kategorien' )
		);
		...
		register_taxonomy( 'produkt-kategorie', 'produkte', $args );
		
}	

add_action( 'init', 'ao_create_anbieter' );

function ao_create_anbieter() {
			
		$labels = array(
				'name' => __( 'Anbieter' ),
				'singular_name' => __( 'Anbieter' )
			);
		$args = array(
			'labels' => $labels,
			'public' => true,
		...
		'capability_type'    => 'post',
			'has_archive' => false,
			'rewrite' => array('slug' => 'apple-business-solution-providers'),
			'taxonomies' => array('post_tag'),
			'supports' => array( 'title', 'custom-fields', 'thumbnail', 'revisions', 'page-attributes', 'post-formats', )
			);

register_post_type( 'anbieter', $args );
}
			// "Anbieter" Custom Taxonomy
			
		add_action( 'init', 'create_anbieter_taxonomies', 0 );
		function create_anbieter_taxonomies() {
		$labels = array(
			'name' => _x( 'Anbieter-Kategorien', 'taxonomy general name' ),
			'singular_name' => _x( 'Anbieter-Kategorie', 'taxonomy singular name' ),
			'search_items' =>  __( 'Anbieter-Kategorien durchsuchen' ),
			'all_items' => __( 'Alle Anbieter-Kategorien' ),
			'parent_item' => __( '&Uuml;bergeordnete Anbieter-Kategorie' ),
			'parent_item_colon' => __( '&Uuml;bergeordnete Anbieter:' ),
			'edit_item' => __( 'Anbieter-Kategorie bearbeiten' ),
			'update_item' => __( 'Anbieter-Kategorie Update' ),
			'add_new_item' => __( 'Neue Anbieter-Kategorie' ),
			'new_item_name' => __( 'Anbieter-Kategorie neu benennen' ),
			'menu_name' => __( 'Anbieter-Kat.' )
		);
		...
		$args = array(
			...
			'show_ui' => true,
			'query_var' => true,
			'rewrite' => array( 'slug' => 'anbieter-kategorie' )
		);
		register_taxonomy( 'anbieter-kategorie', 'anbieter', $args );
		
}	

add_action( 'init', 'ao_create_termine' );

function ao_create_termine() {
		...
		'capability_type'    => 'post',
			'has_archive' => true,
			'rewrite' => array('slug' => 'apple-business-veranstaltungen'),
			'taxonomies' => array('post_tag'),
			'supports' => array( 'title', 'custom-fields', 'thumbnail', 'excerpt', 'editor', 'revisions', 'page-attributes', 'comments', 'post-formats',  )
			);

register_post_type( 'termine', $args );
}
			// "Termine" Custom Taxonomy
			
		add_action( 'init', 'create_termine_taxonomies', 0 );
		function create_termine_taxonomies() {
		$labels = array(
			'name' => _x( 'Termin-Kategorien', 'taxonomy general name' ),
			'singular_name' => _x( 'Termin-Kategorie', 'taxonomy singular name' ),
			'search_items' =>  __( 'Termin-Kategorien durchsuchen' ),
			'all_items' => __( 'Alle Termin-Kategorien' ),
			'parent_item' => __( '&Uuml;bergeordnete Termin-Kategorie' ),
			'parent_item_colon' => __( '&Uuml;bergeordnete Termin:' ),
			'edit_item' => __( 'Termin-Kategorie bearbeiten' ),
			'update_item' => __( 'Termin-Kategorie Update' ),
			'add_new_item' => __( 'Neue Termin-Kategorie' ),
			'new_item_name' => __( 'Termin-Kategorie neu benennen' ),
			'menu_name' => __( 'Termin-Kat.' )
		);
		...
		$args = array(
			...
			'show_ui' => true,
			'query_var' => true,
			'rewrite' => array( 'slug' => 'termine-kategorie' )
		);
		register_taxonomy( 'termine-kategorie', 'termine', $args );
		
}	

add_action( 'init', 'ao_create_loesungen' );

function ao_create_loesungen() {
			
		$labels = array(
				'name' => __( 'L&ouml;sungen' ),
				'singular_name' => __( 'L&ouml;sung' )
			);
		$args = array(
			'labels' => $labels,
			'public' => true,
		...
		'capability_type'    => 'post',
			'has_archive' => false,
			'rewrite' => array('slug' => 'apple-business-solutions'),
			'taxonomies' => array('post_tag'),
			'supports' => array( 'title', 'custom-fields', 'editor', 'thumbnail', 'excerpt', 'revisions', 'page-attributes', 'comments', 'post-formats', 'trackbacks'  )
			);

register_post_type( 'loesungen', $args );
}
			// "Loesungen" Custom Taxonomy
			
		add_action( 'init', 'create_loesungen_taxonomies', 0 );
		function create_loesungen_taxonomies() {
		$labels = array(
			'name' => _x( 'L&ouml;sungs-Kategorien', 'taxonomy general name' ),
			'singular_name' => _x( 'L&ouml;sungs-Kategorie', 'taxonomy singular name' ),
			'search_items' =>  __( 'L&ouml;sungs-Kategorien durchsuchen' ),
			'all_items' => __( 'Alle L&ouml;sungs-Kategorien' ),
			'parent_item' => __( '&Uuml;bergeordnete L&ouml;sungs-Kategorie' ),
			'parent_item_colon' => __( '&Uuml;bergeordnete L&ouml;sung:' ),
			'edit_item' => __( 'L&ouml;sungs-Kategorie bearbeiten' ),
			'update_item' => __( 'L&ouml;sungs-Kategorie Update' ),
			'add_new_item' => __( 'Neue L&ouml;sungs-Kategorie' ),
			'new_item_name' => __( 'L&ouml;sungs-Kategorie neu benennen' ),
			'menu_name' => __( 'L&ouml;sungs-Kat.' )
		);
		...
		$args = array(
			...
			'show_ui' => true,
			'query_var' => true,
			'rewrite' => array( 'slug' => 'loesungen-kategorie' )
		);
		register_taxonomy( 'loesungen-kategorie', 'loesungen', $args );
		
}	

/*-----------------------------------------------------------------------------------*/
/* CUSTOM POST TYPE-KATEGORIEN LISTEN mit folgender Funktion im Template: <?php echo custom_taxonomies_terms_links(); ?> */
/*-----------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------------*/
/* CUSTOM POST TYPES IN DER SUCHE ÜBER TAGS FINDEN */
/*-----------------------------------------------------------------------------------*/
add_filter('pre_get_posts', 'query_post_type');
function query_post_type($query) {
  if(is_category() || is_tag()) {
    $post_type_suche = get_query_var('post_type_suche');
	if($post_type_suche)
	    $post_type_suche = $post_type_suche;
	else
	    $post_type_suche = array('post','produkte','loesungen','termine','anbieter'); // replace cpt to your custom post type
    $query->set('post_type_suche',$post_type_suche);
	return $query;
    }
}
